...
LibGA::FieldEdge::FieldEdge(int sindex, int gindex, Node* _s, Node* _g, float _d)
    : start_index(sindex),
      goal_index(gindex),
      s(_s),
      g(_g),
...
LibGA::FieldEdge::FieldEdge(int sindex, int gindex, Node* _s, Node* _g, float _d1, float _d2)
    : start_index(sindex),
      goal_index(gindex),
      s(_s),
      g(_g),
...
void LibGA::FieldEdge::setRealDist(float _d)
{
  if (!evaled) {
    evaled = true;
    d = _d;
...
LibGA::Matching::Matching(const Nodes& _goals)
    : goals(_goals),
      N(goals.size()),
      adj(N * 2, std::vector<int>()),
      mate(N * 2, NIL),
      cost(N, std::vector<float>(N, NIL)),
      matched_num(0),
...
void LibGA::Matching::halt (const std::string& msg)
{
